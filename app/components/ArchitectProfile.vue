<template>
  <section id="rafael" ref="profileSection" class="relative py-32 px-6 lg:px-12 bg-[#050D1A] scroll-mt-20">
    <div class="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
      ...
      <div class="lg:col-span-4 flex flex-col items-center lg:items-start gap-6">
        <div class="relative w-full max-w-sm rounded-[2rem] overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
          <img
            ref="parallaxImage"
          ...
          />
          <div class="absolute inset-0 ring-1 ring-inset ring-white/10 rounded-[2rem]"></div>
        </div>

        <!-- Name & Role -->
        <div class="text-center lg:text-left">
          <div class="text-white font-semibold text-xl">Rafael Mauricio Rivera Álvarez</div>
          <div class="text-primary-light text-sm mt-1">El Arquitecto Financiero</div>
        </div>

        <!-- Credentials -->
        <div class="w-full bg-neutral-900/60 border border-neutral-800 rounded-2xl p-5 grid grid-cols-2 gap-2">
          <div v-for="cred in credentials" :key="cred" class="flex items-center gap-2 text-xs text-neutral-300">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-primary-light shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
            {{ cred }}
          </div>
        </div>
      </div>
      ...
      <div ref="bioContainer" class="lg:col-span-8 flex flex-col items-start">
        <div class="bio-element inline-block px-4 py-1.5 rounded-full bg-primary/10 text-primary-light text-xs font-semibold tracking-widest uppercase mb-5 opacity-0 translate-y-6">
          El arquitecto
        </div>

        <h2 class="bio-element font-serif font-normal text-3xl md:text-4xl lg:text-[2.6rem] leading-[1.2] tracking-tight mb-8 text-white opacity-0 translate-y-6">
          El contador que conoce el camino porque también lo recorrió.
        </h2>

        <div class="space-y-5 text-base md:text-lg text-neutral-400 font-light leading-relaxed">
        ...
        <div class="bio-element mt-10 border-l-2 border-primary pl-6 opacity-0 translate-y-6">
          <p class="text-white font-serif text-xl md:text-2xl italic leading-snug">
            "La diferencia entre una hipoteca de 30 años y una de 12 no es cuánto ganas. Es cómo aplicas cada peso que ya tienes."
          </p>
          <p class="mt-3 text-primary-light text-sm font-semibold">— Rafael Rivera · El Arquitecto Financiero</p>
        </div>
      </div>
